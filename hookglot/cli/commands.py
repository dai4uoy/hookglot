...
def cmd_install(args):
    ...
    print(colored("\n📖 Viewing your conversation log:", "cyan"))
    print(f"  Translations are saved to {colored('~/.hookglot/conversation.md', 'green')}")
    print(f"  (and per-session in {colored('~/.hookglot/conversations/', 'green')}).")
    print(f"  They are {colored('NOT shown inline in Claude Code', 'yellow')} to avoid token leak.")
    print()
    print(f"  {colored('Recommended methods:', 'cyan')}")
    print(f"    • {colored('hookglot start', 'green')}                    grip server + auto-open browser")
    print(f"    • {colored('/hookglot-start', 'green')}                   same, from inside Claude Code")
    print()
    print(f"  {colored('Manual viewing:', 'cyan')}")
    print(f"    • {colored('grip ~/.hookglot/conversation.md', 'green')}  run grip yourself")
    print(f"    • {colored('code ~/.hookglot/conversation.md', 'green')}  VSCode markdown preview")
    print()
    print(f"  {colored('Live tail (separate terminal):', 'cyan')}")
    print(f"    • PowerShell: {colored('Get-Content ~/.hookglot/conversation.md -Wait -Tail 0', 'green')}")
    print(f"    • Linux/Mac:  {colored('tail -f ~/.hookglot/conversation.md', 'green')}")
...
def cmd_start(args):
    ...
    if not conv_file.exists():
        print(colored("ℹ️  No conversation.md yet — chat with Claude first.", "cyan"))
        return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  Translations are saved to ~/.hookglot/conversation.md (cumulative)\n"
            "  and ~/.hookglot/conversations/<datetime>.md (per session).\n"
            "  They are NOT shown inline in Claude Code (would leak tokens).\n"
            "\n"
            "  Recommended viewing methods:\n"
            "    hookglot start                # launch grip + browser\n"
            "    /hookglot-start               # inside Claude Code\n"
            "    grip ~/.hookglot/conversation.md     # manual\n"
            "    code ~/.hookglot/conversation.md     # VSCode preview\n"
            "\n"
